...
trait Tables {
  ...
  lazy val schema: profile.SchemaDescription =
    PlayEvolutions.schema ++ Version.schema
  ...
  implicit def GetResultPlayEvolutionsRow(implicit
    e0: GR[Int],
    e1: GR[String],
  ...
  class PlayEvolutions(_tableTag: Tag)
      extends profile.api.Table[PlayEvolutionsRow](
        _tableTag,
        "play_evolutions"
      ) {
    def * =
    ...
    def ? =
      (
        ...
        { r =>
          import r._;
          _1.map(_ =>
            PlayEvolutionsRow.tupled((_1.get, _2.get, _3.get, _4, _5, _6, _7))
          )
        },
        (_: Any) =>
    ...
    val hash: Rep[String] =
      column[String]("hash", O.Length(255, varying = true))

    /** Database column applied_at SqlType(timestamp) */
    val appliedAt: Rep[java.sql.Timestamp] =
      column[java.sql.Timestamp]("applied_at")

    /** Database column apply_script SqlType(text), Default(None) */
    val applyScript: Rep[Option[String]] =
      column[Option[String]]("apply_script", O.Default(None))

    /** Database column revert_script SqlType(text), Default(None) */
    val revertScript: Rep[Option[String]] =
      column[Option[String]]("revert_script", O.Default(None))

    /** Database column state SqlType(varchar), Length(255,true), Default(None) */
    val state: Rep[Option[String]] = column[Option[String]](
      "state",
    ...
    val lastProblem: Rep[Option[String]] =
      column[Option[String]]("last_problem", O.Default(None))
  ...
  implicit def GetResultVersionRow(implicit
    e0: GR[Int],
    e1: GR[String],
  ...
  class Version(_tableTag: Tag)
      extends profile.api.Table[VersionRow](_tableTag, "version") {
    def * =
      (id, appVersion, createdAt) <> (VersionRow.tupled, VersionRow.unapply)

    /** Maps whole row to an option. Useful for outer joins. */
    def ? =
      ((Rep.Some(id), Rep.Some(appVersion), createdAt)).shaped.<>(
        { r =>
          import r._; _1.map(_ => VersionRow.tupled((_1.get, _2.get, _3)))
        },
        (_: Any) =>
          throw new Exception("Inserting into ? projection not supported.")
      )
    ...
    val appVersion: Rep[String] =
      column[String]("app_version", O.Length(10, varying = true))

    /** Database column created_at SqlType(timestamptz) */
    val createdAt: Rep[Option[java.sql.Timestamp]] =
      column[Option[java.sql.Timestamp]]("created_at")
